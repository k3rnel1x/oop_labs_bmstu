...
void QtSceneDrawer::DrawScene(const Scene& scene)
{
    ...
    for (auto figure : scene.GetFigures())
    {
        ...
        for (auto edge : figure.GetEdges())
        {
            ...
            // if (begin.first < _canvas->width() && begin.second < _canvas->height()
            // &&  end.first < _canvas->width()   && end.second < _canvas->height())
            // {
...
std::pair<int, int> QtSceneDrawer::_processVertex(const Vertex& vertex, Figure::FigureMetaInfo& info)
{
    ...
    point.x = SCALE*(-1 + ( point.x - info.xMin ) / ( info.xMax - info.xMin ) * 2);
    point.y = SCALE*(-1 + ( point.y - info.yMin ) / ( info.yMax - info.yMin ) * 2);
    point.z = -1 + (point.z)/( 50 )*2 + INITZ;
    ...
    ipair.first =  int( (p.first  + 1.0)/2.0  * _canvas->width() );
    ipair.second = int( (p.second + 1.0)/2.0 * _canvas->height() );
